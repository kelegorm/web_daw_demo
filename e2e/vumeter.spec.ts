...
test('VU meter RMS base bar is green and warm colors are drawn only in upper segments', async ({ page }) => {
  await page.goto('/')
  ...
  // Base RMS fill starts in green tones.
  expect(bgStyle.includes('47, 127, 86') || bgStyle.includes('105, 199, 136')).toBe(true)
})
...
test('VU meter peak hold tick appears after note is played and uses a zone color', async ({ page }) => {
  ...
  const zoneRgbFragments = ['105, 199, 136', '245, 213, 74', '232, 59, 59']
